...
if __name__=="__main__":
    ...
    times,voltages_in,voltages_out,freqs,resistance=load_freqsweep('/home/thouin/Documents/Repo/colberto/piezo_od117_59_impedance_measurement.h5')
    impedances=extract_impedances(times,voltages_in,voltages_out,freqs,resistance)
    plt.loglog(freqs,np.real(impedances),label='OD1 thin Piezo holder')
   # #plt.semilogx(freqs,np.imag(impedances),label='imag_beforetight')
    times,voltages_in,voltages_out,freqs,resistance=load_freqsweep('/home/thouin/Documents/Repo/colberto/piezo_od1_noair16_34_impedance_measurement.h5')
    impedances=extract_impedances(times,voltages_in,voltages_out,freqs,resistance)
    plt.loglog(freqs,np.real(impedances),label='real, OD1 thick piezo holder')
    ...
    #times,voltages_in,voltages_out,freqs,resistance=load_freqsweep('/home/thouin/Documents/Repo/colberto/piezo_3mm_tighter_damping17_11_impedance_measurement.h5')
    #impedances=extract_impedances(times,voltages_in,voltages_out,freqs,resistance)
    #plt.semilogx(freqs,np.real(impedances),label='real, 3mm mirror damper tighter')
