...
1. TO PERFORM A CHIRP SCAN, THE STRESING CAMERA MUST BE IN CONTINUOUS MODE. This is set in the Acquisition panel of the Spectrum View tab: pick the `continuous` mode, which puts the board on its own timer, and set the scan interval (the integration time) to something like 20 ms. The panel writes the underlying `Scan_Trig`/`Block_Trig` registers for you and shows the resulting rate and total acquisition time. In trigger mode you won't measure anything. The number of samples per block can be reduced to <10 (>2) to avoid measuring many times. Comments:

- Without DOE (only one beam with higher intensity), a 20 ms scan interval gives a good chirp scan. 
- With DOE (only one of the four beams), 50 ms seems ok. 
...
4. Then you can click on `acquire`. If the demo mode checkbox of the panel is ticked, it will take old saved data. Otherwise, the program will perform the chirp scan and show you the result on the top right figure. The background saved data will be subtracted from every scan step. 
...
1. To perform a delay scan, the camera can be in triggering mode. Select the `external` mode in the Acquisition panel of the Spectrum View tab, which takes one readout per laser pulse.
...
4. An iris selecting the ONLY the cross correlation SHG is needded for an accurate measure of the beams duration. Then you can click on `Acquire`. If the demo mode checkbox of the panel is ticked, it will take old saved data. Otherwise, the program will perform the delay scan and show you the result on the top right figure. The background saved data will be subtracted from every scan step. 
